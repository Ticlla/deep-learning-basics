# Level 4: Convolutional Neural Networks (CNNs)

> **Goal**: Understand how CNNs exploit spatial structure in images to achieve state-of-the-art accuracy (~99%+)

---

## 📚 Overview

CNNs are the breakthrough architecture that revolutionized computer vision. They achieve better results than fully-connected networks by:

1. **Understanding spatial relationships** in images
2. **Sharing weights** across the image (fewer parameters)
3. **Building translation invariance** (detect features anywhere)

```
┌──────────────────────────────────────────────────────────────────────────┐
│                         CNN vs Fully Connected                            │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│  Fully Connected (Level 3):                                               │
│  ┌───────────────┐      ┌─────────────┐      ┌──────────┐                │
│  │ 784 inputs    │──────│ 100 hidden  │──────│ 10 output│                │
│  │ (flattened)   │      │ (dense)     │      │          │                │
│  └───────────────┘      └─────────────┘      └──────────┘                │
│  Parameters: 784×100 + 100×10 = 79,400                                   │
│  Accuracy: ~97.7%                                                         │
│                                                                           │
│  CNN (Level 4):                                                           │
│  ┌───────────┐   ┌──────────┐   ┌──────────┐   ┌────────┐   ┌────────┐  │
│  │ 28×28     │───│Conv 5×5  │───│Pool 2×2  │───│ FC 100 │───│ 10 out │  │
│  │ image     │   │20 filters│   │12×12×20  │   │        │   │        │  │
│  └───────────┘   └──────────┘   └──────────┘   └────────┘   └────────┘  │
│  Parameters: 20×(5×5) + 20×12×12×100 + ... ≈ 30,000                      │
│  Accuracy: ~99%+ ⭐                                                       │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

---

## 🧩 Key Concepts

### 1. Local Receptive Fields

Instead of connecting every pixel to every neuron, CNNs use **small windows** (filters) that scan across the image:

```
Input Image (28×28)                    First Hidden Neuron
┌─────────────────────┐                     ┌───┐
│ ┌─────┐             │                     │   │
│ │ 5×5 │ ─────────────────────────────────→│ ○ │
│ │field│             │    25 weights       │   │
│ └─────┘             │    + 1 bias         └───┘
│                     │                   
│                     │                    Second Hidden Neuron
│   ┌─────┐           │                     ┌───┐
│   │ 5×5 │ ───────────────────────────────→│ ○ │
│   │     │           │                     │   │
│   └─────┘           │                     └───┘
└─────────────────────┘                    
    ↑                                      (same weights!)
    Stride = 1 pixel
```

**Key insight**: The 5×5 window slides across the entire image, creating a **feature map** (24×24 for 28×28 input with 5×5 filter).

### 2. Weight Sharing (Shared Filters)

The SAME weights are used for every position in the image:

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         WEIGHT SHARING                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   Filter (5×5 = 25 weights + 1 bias):                                   │
│   ┌─────────────────┐                                                   │
│   │ w₁  w₂  w₃  w₄  w₅  │                                              │
│   │ w₆  w₇  w₈  w₉  w₁₀ │     These 26 values are used                 │
│   │ w₁₁ w₁₂ w₁₃ w₁₄ w₁₅│     for ALL positions!                       │
│   │ w₁₆ w₁₇ w₁₈ w₁₉ w₂₀│                                              │
│   │ w₂₁ w₂₂ w₂₃ w₂₄ w₂₅│                                              │
│   └─────────────────┘                                                   │
│                                                                          │
│   Without sharing: 24×24 positions × 26 = 14,976 parameters             │
│   With sharing:    26 parameters only!   ← 576× fewer!                  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

**Why it works**: A feature (like an edge or curve) should be detectable anywhere in the image.

### 3. Feature Maps

One filter learns ONE feature. We use MULTIPLE filters to learn different features:

```
Input (28×28)     20 Filters (5×5 each)        20 Feature Maps (24×24 each)
                                              
┌───────────┐     ┌───────────────────┐       ┌───────────┬───────────┬───┐
│           │     │ Filter 1: ─      │       │ Map 1     │ Map 2     │...│
│   "7"     │ ──→ │ Filter 2: |      │  ──→  │ (edges)   │ (curves)  │   │
│           │     │ Filter 3: /      │       │           │           │   │
│           │     │ ...              │       │           │           │   │
│           │     │ Filter 20: ○     │       │           │           │   │
└───────────┘     └───────────────────┘       └───────────┴───────────┴───┘

                  Each filter: 26 params      Output: 24×24×20 = 11,520 values
                  Total: 20×26 = 520 params
```

### 4. Pooling Layers (Max Pooling)

After convolution, we **downsample** using max pooling:

```
┌──────────────────────────────────────────────────────────────────────────┐
│                           MAX POOLING (2×2)                               │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│   Feature Map (24×24)              Pooled Map (12×12)                     │
│   ┌─────────────────┐              ┌─────────────────┐                   │
│   │ 1  3 │ 2  4 │...│              │                 │                   │
│   │ 0  2 │ 1  3 │   │     max      │  3    4   ...   │                   │
│   ├──────┼──────┤   │  ────────→   │                 │                   │
│   │ 5  6 │ 7  8 │   │    2×2       │  6    8   ...   │                   │
│   │ 4  1 │ 2  5 │   │              │                 │                   │
│   └─────────────────┘              └─────────────────┘                   │
│         ↑                                   ↑                             │
│   Take max from                       Half the size!                      │
│   each 2×2 region                    (24/2 = 12)                         │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

**Benefits**:
- Reduces computation (smaller feature maps)
- Adds **translation invariance** (small shifts don't change output)
- Controls overfitting

### 5. ReLU Activation

CNNs typically use **ReLU** instead of sigmoid:

```
┌──────────────────────────────────────────────────────────────────────────┐
│                         ACTIVATION FUNCTIONS                              │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│   Sigmoid: σ(z) = 1/(1+e^(-z))         ReLU: f(z) = max(0, z)            │
│                                                                           │
│       1.0 ┤      ╭────────             ∞  ┤           /                  │
│           │     /                         │          /                   │
│       0.5 ┤    │                          │         /                    │
│           │   /                           │        /                     │
│       0.0 ┼───────────────             0  ┼───────●─────────            │
│           -4    0    4                      -4    0    4                  │
│                                                                           │
│   Problem: Vanishing gradient         ✓ No vanishing gradient            │
│            when |z| is large          ✓ Computationally cheap            │
│                                       ✓ Sparse activations               │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

### 6. Dropout Regularization

During training, randomly "drop" neurons:

```
┌──────────────────────────────────────────────────────────────────────────┐
│                              DROPOUT                                      │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│   Training (p=0.5):                    Testing:                           │
│                                                                           │
│   ○ ────────→ ○                       ○ ────────→ ○                      │
│   ○ ────╳     ●  (dropped)            ○ ────────→ ○  (×0.5)              │
│   ○ ────────→ ○                       ○ ────────→ ○                      │
│   ○ ────╳     ●  (dropped)            ○ ────────→ ○  (×0.5)              │
│   ○ ────────→ ○                       ○ ────────→ ○                      │
│                                                                           │
│   Each training step uses              At test time, use all              │
│   a different random subset           neurons but scale outputs          │
│                                                                           │
│   Effect: Prevents co-adaptation, like training ensemble of networks     │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

### 7. Softmax Output Layer

For classification, use softmax to get probabilities:

```
┌──────────────────────────────────────────────────────────────────────────┐
│                              SOFTMAX                                      │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│   softmax(zⱼ) = e^(zⱼ) / Σₖ e^(zₖ)                                       │
│                                                                           │
│   Raw outputs (logits):        After softmax:                             │
│   ┌────────────────────┐       ┌────────────────────┐                    │
│   │ z₀ =  2.1         │       │ P(0) = 0.01        │                    │
│   │ z₁ = -0.5         │       │ P(1) = 0.00        │                    │
│   │ z₂ =  0.3         │       │ P(2) = 0.02        │                    │
│   │ z₃ =  1.2         │       │ P(3) = 0.05        │                    │
│   │ z₄ = -1.0         │       │ P(4) = 0.01        │                    │
│   │ z₅ =  0.8         │  ──→  │ P(5) = 0.03        │                    │
│   │ z₆ =  3.9         │       │ P(6) = 0.73  ← max │                    │
│   │ z₇ =  2.5         │       │ P(7) = 0.18        │                    │
│   │ z₈ = -0.2         │       │ P(8) = 0.01        │                    │
│   │ z₉ =  0.1         │       │ P(9) = 0.02        │                    │
│   └────────────────────┘       └────────────────────┘                    │
│                                     Sum = 1.00                            │
│                                                                           │
│   Used with Cross-Entropy Loss: L = -log(P(correct class))               │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

---

## 🏗️ Complete CNN Architecture

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                         COMPLETE CNN ARCHITECTURE                             │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                               │
│  INPUT              CONV1 + POOL1       CONV2 + POOL2       FC + OUTPUT      │
│  (1×28×28)          (20×12×12)          (40×4×4)            (100) → (10)     │
│                                                                               │
│  ┌─────────┐       ┌──────────────┐    ┌──────────────┐    ┌────────────┐   │
│  │  ░░░░░  │       │ ░ ░ ░ ░     │    │ ░░░░         │    │            │   │
│  │  ░███░  │ ───→  │ ░░░░░░      │ ─→ │ ░░░░         │ ─→ │  FC: 100   │ ─→│
│  │  ░░█░░  │ Conv  │ ░░░░░░      │Conv│ ░░░░         │    │  neurons   │   │
│  │  ░░█░░  │ 5×5   │ ...         │5×5 │              │    │            │   │
│  │  ░░█░░  │       │             │    │              │    │  Dropout   │   │
│  └─────────┘       └──────────────┘    └──────────────┘    └────────────┘   │
│       │                  │                    │                   │          │
│       │            20 filters           40 filters          Softmax          │
│       │            + ReLU               + ReLU              Output          │
│       │            + MaxPool 2×2        + MaxPool 2×2       (10 classes)    │
│       │                                                                      │
│       │                                                                      │
│  784 pixels        20×(5×5×1) = 500     40×(5×5×20) = 20,000                │
│                    → 20×12×12 = 2,880   → 40×4×4 = 640                      │
│                                                                               │
└──────────────────────────────────────────────────────────────────────────────┘

Dimension calculations:
- Input: 28×28 → Conv 5×5 → 24×24 → Pool 2×2 → 12×12
- Conv2: 12×12 → Conv 5×5 → 8×8 → Pool 2×2 → 4×4
- Flatten: 40×4×4 = 640 → FC → 100 → 10
```

---

## 📈 Performance Progression

| Architecture | Accuracy | Parameters |
|-------------|----------|------------|
| FC [784→100→10] (Level 3) | ~97.7% | ~80K |
| Conv(20) → FC(100) → 10 | ~98.5% | ~30K |
| Conv(20) → Conv(40) → FC(100) → 10 | ~99.0% | ~35K |
| + ReLU activation | ~99.2% | ~35K |
| + Dropout + 1000 FC neurons | ~99.6% | ~800K |

---

## 🐍 PyTorch Implementation

```python
import torch
import torch.nn as nn
import torch.nn.functional as F

class CNN(nn.Module):
    def __init__(self):
        super(CNN, self).__init__()
        # Convolutional layers
        self.conv1 = nn.Conv2d(1, 20, kernel_size=5)   # 28→24
        self.pool = nn.MaxPool2d(2, 2)                 # 24→12
        self.conv2 = nn.Conv2d(20, 40, kernel_size=5)  # 12→8→4
        
        # Fully connected layers
        self.fc1 = nn.Linear(40 * 4 * 4, 100)
        self.dropout = nn.Dropout(0.5)
        self.fc2 = nn.Linear(100, 10)
    
    def forward(self, x):
        # Conv block 1: Conv → ReLU → Pool
        x = self.pool(F.relu(self.conv1(x)))  # 1×28×28 → 20×12×12
        
        # Conv block 2: Conv → ReLU → Pool
        x = self.pool(F.relu(self.conv2(x)))  # 20×12×12 → 40×4×4
        
        # Flatten for FC layers
        x = x.view(-1, 40 * 4 * 4)             # Flatten
        
        # FC layers with dropout
        x = F.relu(self.fc1(x))               # 640 → 100
        x = self.dropout(x)                    # Dropout
        x = self.fc2(x)                        # 100 → 10
        
        return F.log_softmax(x, dim=1)        # Softmax output
```

---

## 🔬 Exploration Scripts

| Script | Description |
|--------|-------------|
| `src/level4/1_cnn_pytorch.py` | Modern PyTorch CNN implementation + training |
| `src/level4/2_interactive_cnn.py` | Interactive demo - draw digits and see CNN predictions |

### Running the Scripts

```bash
# Train the CNN (saves model to models/level4_cnn.pt)
cd /home/alcidesticlla/Documents/MOOC/mniels/neural-networks-and-deep-learning/src
python level4/1_cnn_pytorch.py

# Run interactive demo (requires trained model)
python level4/2_interactive_cnn.py
```

---

## 🖌️ Interactive Demo Features

The `2_interactive_cnn.py` demo includes:

### MNIST-Style Preprocessing
Hand-drawn digits look different from MNIST training data. The demo applies preprocessing to improve recognition:

```python
def preprocess_image(self):
    """
    MNIST-style centering for better accuracy:
    1. Find bounding box of the digit
    2. Crop to just the digit
    3. Resize to fit in 20×20 box (preserving aspect ratio)
    4. Center in 28×28 image
    5. Apply Gaussian blur for anti-aliasing
    """
```

### Features
- **Real-time predictions** as you draw
- **28×28 preview** showing what the CNN actually sees
- **Confidence bars** for all 10 digit classes
- **Model accuracy display** (~99.29% on MNIST test set)

---

## 🎓 Key Takeaways

1. **CNNs exploit spatial structure** - pixels near each other are related
2. **Weight sharing reduces parameters** - same filter everywhere
3. **Pooling adds invariance** - small shifts don't matter
4. **ReLU prevents vanishing gradients** - better than sigmoid for deep nets
5. **Dropout prevents overfitting** - like training many networks
6. **Softmax gives probabilities** - outputs sum to 1
7. **Preprocessing matters** - centering hand-drawn digits improves recognition

---

*Try the interactive CNN demo: `python level4/2_interactive_cnn.py`*

